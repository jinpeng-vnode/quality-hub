  ...
  <div class="testcase-view">
    ...
    <a-drawer v-model:open="showDrawer" :title="editingCase ? '编辑测试用例' : '新建测试用例'" width="640" @close="resetForm">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="Playwright 脚本" required>
      ...
      <template #footer>
        <div style="display: flex; justify-content: space-between;">
          <a-space>
            <a-button v-if="editingCase" :loading="executing" @click="executeSingle" type="dashed">
...
export default defineComponent({
  ...
  setup() {
    ...
    const filters = reactive({ featureId: undefined as string | undefined })
    const form = reactive({
    ...
    function editCase(record: TestCase) {
      ...
      formSteps.value = record.steps && record.steps.length > 0 ? [...record.steps] : ['']
    ...
    async function handleSubmit() {
      ...
      if (!form.title.trim() || !form.featureId || stepsArray.length === 0 || !form.expectedResult.trim() || !form.midsceneScript.trim()) {
        message.warning('请填写所有必填项（标题、功能点、测试步骤、预期结果、脚本）')
        return
      }
      const payload = {
        ...
        midsceneScript: form.midsceneScript,
      }
      try {
    ...
    onMounted(() => {
      ...
      if (queryFeatureId) filters.featureId = String(queryFeatureId)
      fetchCases()
    })
    ...
    return { testCases, features, loading, showDrawer, editingCase, filters, form, formSteps, columns, priorityColor, priorityText, copyId, openCreate, editCase, resetForm, handleSubmit, deleteCase, fetchCases, executing, showResult, execResult, executeSingle }
  },
})
</script>
...
.id-cell { cursor: pointer; color: #1890ff; }
